...
# ============================================================
#  FUNCIONES TÉCNICAS (BLUR, RECORTE, PDF) - NO TOCAR
# ============================================================
...
#  INICIO APP
# ============================================================
st.set_page_config(page_title="Provident Pro v160", layout="wide", initial_sidebar_state="expanded")
...
<style>
    ...
    .master-container {
        ...
        max-width: 400px;
        margin: 0 auto;
        display: grid; /* Obliga a que los hijos se encimen */
        grid-template-columns: 1fr;
    }

    /* CAPA 0: LA TABLA (EL FONDO) */
    .cal-grid-table {
        grid-area: 1 / 1 / 2 / 2; /* Ocupa la misma celda del grid */
        width: 100%;
    ...
    .cell-day-num { background: #00b0f0; color: white; font-weight: bold; font-size: 0.85em; text-align: center; height: 16px; }
    ...
    /* CAPA 1: LOS BOTONES (EL FRENTE) */
    .buttons-layer {
        grid-area: 1 / 1 / 2 / 2; /* Misma celda del grid: QUEDA ENCIMA */
        z-index: 10;
        margin-top: 26px; /* Para no tapar los headers L-M-X... */
        pointer-events: none;
    }
    
    /* Forzar los botones a ser transparentes y cubrir la celda */
    .buttons-layer [data-testid="column"] { padding: 0 !important; }
    .buttons-layer button {
        ...
        pointer-events: auto; /* Solo los botones reciben clicks */
        margin: 0 !important;
        padding: 0 !important;
    }
...
if 'raw_records' not in st.session_state:
    ...
    if mod == "Calendario":
        ...
        if st.session_state.dia_seleccionado:
            ...
            jwa = f"<script>function c(){{navigator.clipboard.writeText(`{msj}`).then(()=>{{window.open('{gp['link']}','_blank');}});}}</script><div onclick='c()' style='background:#25D3:#25D366;color:white;padding:12px;text-align:center;border-radius:10px;cursor:pointer;font-weight:bold;'>📲 WhatsApp {gp['name']}</div>"
            if gp['link']: st.components.v1.html(jwa, height=80)
        
        else:
            # --- VISTA MES (SUPERPOSICIÓN REAL) ---
            if fechas_oc:
                dt_ref = datetime.strptime(list(fechas_oc.keys())[0], '%Y-%m-%d')
                weeks = calendar.Calendar(0).monthdayscalendar(dt_ref.year, dt_ref.month)

                st.markdown(f"<div style='background:linear-gradient(135deg,#002060,#00b0f0);padding:10px;border-radius:8px;text-align:center;color:white;font-weight:bold;text-transform:uppercase;margin: 0 auto 10px auto; max-width:400px;'>{MESES_ES[dt_ref.month-1]} {dt_ref.year}</div>", unsafe_allow_html=True)
                
                # CONTENEDOR GRID
                st.markdown('<div class="master-container">', unsafe_allow_html=True)
                
                # CAPA 0 (FONDO): TABLA HTML
                ...
                # CAPA 1 (FRENTE): BOTONES DE STREAMLIT
                        ...
                        if day > 0:
                            ...
                            with cols[i]:
                                if fk in fechas_oc:
                                    ...
                                    st.write("") # Mantiene el espacio en la columna
                st.markdown('</div></div>', unsafe_allow_html=True)

    elif mod in ["Postales", "Reportes"]:
        # (Lógica de generación íntegra de versiones anteriores...)
            ...
            if st.button("🚀 PROCESAR PACK"):
                ...
                for i, ix in enumerate(idx_list):
                    ...
                    try:
                        ...
                        # Procesar formas y fotos (Lógica sagrada)
                ...
                if zip_data:
                    ...
                    st.download_button("⬇️ DESCARGAR ZIP", z_buf.getvalue(), f"Provident_Pack.zip", "application/zip")
