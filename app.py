...
# --- UI ---
st.set_page_config(page_title="Provident Pro v28", layout="wide")
st.title("🚀 Generador Pro: Barra de Progreso y Texto Plano")
...
if 'raw_records' in st.session_state:
    ...
    if sel_idx:
        ...
        if st.button("🔥 GENERAR ARCHIVOS", use_container_width=True, type="primary"):
            p_bar = st.progress(0) # BARRA RESTAURADA
            status_text = st.empty() # DETALLE RESTAURADO
            zip_buf = BytesIO()
            total = len(sel_idx)
            ...
            st.download_button("📥 DESCARGAR ZIP", zip_buf.getvalue(), "Provident_Final.zip", use_container_width=True)
