...
# --- UI ---
st.set_page_config(page_title="Provident Pro v30", layout="wide")
st.title("🚀 Generador Pro: Fix Attachments & 12/14/11pts")
...
with st.sidebar:
    if st.button("💾 GUARDAR JSON", use_container_width=True, type="primary"):
        guardar_config_json(st.session_state.config); st.toast("Vínculos guardados")
    st.divider()
    r_bases = requests.get("https://api.airtable.com/v0/meta/bases", headers=headers)
    if r_bases.status_code == 200:
        base_sel = st.selectbox("Base:", [""] + [b['name'] for b in r_bases.json()['bases']])
        if base_sel:
            base_id = next(b['id'] for b in r_bases.json()['bases'] if b['name'] == base_sel)
            r_tab = requests.get(f"https://api.airtable.com/v0/meta/bases/{base_id}/tables", headers=headers)
            tabla_sel = st.selectbox("Tabla:", [t['name'] for t in r_tab.json()['tables']])
...
if 'raw_records' in st.session_state:
    ...
    # Ocultar columnas de adjuntos en la tabla para limpieza visual
    ...
    if sel_idx:
        ...
        if st.button("🔥 GENERAR", use_container_width=True, type="primary"):
                ...
                for i, idx in enumerate(sel_idx):
                    ...
                    for slide in prs.slides:
                        for shape in slide.shapes:
                            # --- 1. PROCESAR IMÁGENES (ANTES DE BORRAR TEXTO) ---
                                        ...
                                        if adj and isinstance(adj, list):
                                            try:
                                                ...
                                                if r_img.status_code == 200:
                                                    ...
                                                    # Borrar el tag de texto para que no se vea debajo de la foto
                                                    shape.text_frame.clear()
                                            except: pass

                            # --- 2. PROCESAR REEMPLAZOS DE TEXTO ---
                            if shape.has_text_frame:
            ...
            status.success(f"✅ ¡{total} archivos generados exitosamente!")
            st.download_button("📥 DESCARGAR ZIP", zip_buf.getvalue(), "Provident_v30.zip", use_container_width=True)
